...
class World(object):
    def __init__(self, cx, cy):
        ...
        self.paused = True
        self.showinfo = True
        # Cohesion/Sepertion/Alignment Variables 
        self.cohesion = 0.0
        self.seperation = 0.0
        self.alignment = 0.0
        self.radius = 10.0
    ...
    def render(self):
        ...
        if self.showinfo:
            ...
            egi.red_pen()
            egi.text_at_pos(0, 20, infotext)

            infotext = ': '.join(('Separation', str(self.seperation)))
            egi.red_pen()
            egi.text_at_pos(0, 30, infotext)
            
            infotext = ': '.join(('Alignment', str(self.alignment)))
            egi.red_pen()
            egi.text_at_pos(0, 40, infotext)
            
            infotext = ': '.join(('Radius', str(self.radius)))
            egi.red_pen()
            egi.text_at_pos(0, 50, infotext)
